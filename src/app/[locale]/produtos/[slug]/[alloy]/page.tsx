...
import { alloyBySlug, productFamilies } from "@/data/products";
        ...
        <Container>
          ...
          <div className="mt-6 grid gap-12 md:grid-cols-[1.1fr_1fr] md:items-center">
            <div>
              ...
              {standards.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2 font-mono text-[11px] uppercase tracking-[0.15em] text-slate-600">
                  {standards.map((s) => (
                    <span
                      key={s}
        ...
        <section className="py-20">
          <Container className="grid gap-12 lg:grid-cols-2">
                ...
                <div className="surface-card mt-5 overflow-hidden rounded-lg">
                  <table className="min-w-full text-left text-sm">
                    <tbody className="divide-y divide-slate-200">
                      {alloyData.composition.map((row) => (
                        <tr key={row.element.en} className="hover:bg-slate-50/60">
                          <th
                            scope="row"
                            className="w-1/2 px-4 py-3 font-medium text-ink"
                          >
                            {row.element[loc]}
                          </th>
            ...
            {alloyData.properties && (
              <div>
                <SectionEyebrow>
                  {loc === "pt" ? "Propriedades físicas e mecânicas" : "Physical & mechanical properties"}
                </SectionEyebrow>
                <h2 className="mt-3 text-2xl font-semibold text-ink">
                  {loc === "pt" ? "Propriedades" : "Properties"}
                </h2>
                <div className="surface-card mt-5 overflow-hidden rounded-lg">
                  <table className="min-w-full text-left text-sm">
                    <tbody className="divide-y divide-slate-200">
                      {alloyData.properties.map((p) => (
                        <tr key={p.label.en} className="hover:bg-slate-50/60">
                          <th
                            scope="row"
                            className="w-1/2 px-4 py-3 font-medium text-ink"
                          >
        ...
        <section className="border-t border-slate-200 bg-paper-soft py-20">
          <Container className="grid gap-12 md:grid-cols-[1.3fr_1fr]">
            ...
            {alloyData.processes && (
              <div>
                <SectionEyebrow>
                  {loc === "pt" ? "Processos" : "Manufacturing processes"}
                </SectionEyebrow>
                <h2 className="mt-3 text-2xl font-semibold text-ink">
                  {loc === "pt" ? "Processos" : "Processes"}
                </h2>
